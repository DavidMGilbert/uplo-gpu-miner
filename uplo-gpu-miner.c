/*
 * A cross-platform GPU miner built for Uplo
 * using OpenCL for interacting with the graphics card
 * and libcurl for interacting with the Uplo daemon.
 */
...
// Given a number of cycles per iter, grind nonces will poll Uplo for a block
...
double grindNonces(int cycles_per_iter) {
	...
	if (target[0] != 0 || target[1] != 0) {
		...
		printf("Received corrupt target from Uplo\n");
...
int main(int argc, char *argv[]) {
	...
	for (i = 1; i < argc; i++) {
		...
		case 'h':
			printf("\nUsage:\n\n");
			printf("\t C - cycles per iter: Number of kernel executions between Uplo API calls and hash rate updates\n");
			...
			printf("\n");
			printf("\t H - host: which host name to use when talking to the uplod api. (default: %s)\n", hostname);
			printf("\n");
			printf("\t P - port: which port to use when talking to the uplod api. (e.g. -p :8480)\n");
			printf("\n");
		...
		case 'P':
			if (++i >= argc) {
				printf("Please pass in a port number following your flag (e.g. -P :8480)\n");
				exit(1);
			}
			if (strlen(argv[i]) < 6) {
	...
	fflush(stdout);
	FILE *fp;
	const char fileName[] = "./uplo-gpu-miner.cl";
	size_t source_size;
	char *source_str;
	fp = fopen(fileName, "r");
	if (!fp) {
